# utils/gpu_kernels.py - ER + XZ + YZ + Global + Vertexing + Dimuon wrappers (shared libs)
import numpy as np
import cupy as cp
import os
import ctypes
...
def run_event_reduction(hit_data, hit_flags, hastoomanyhits, datasizes):
    ...
    # convention used by your ER kernel: datasizes[0] stores nEvents
...
def run_yz_tracking(hit_vars, gplane_array, hastoomany_trim, tracks_xz, ntracks_xz):
    ...
    # YZ kernel writes outputs into same index as the XZ seed index.
...
# ---------------- Vertexing ----------------
_lib_vtx = ctypes.CDLL(os.path.join(os.path.dirname(__file__), "libvertexing.so"))
_lib_vtx.launch_gKernel_Vertexing.restype = None
_lib_vtx.launch_gKernel_Vertexing.argtypes = [
    ctypes.c_void_p,  # dTracks
    ctypes.c_void_p,  # dNTracks
    ctypes.c_void_p,  # dPlane
    ctypes.c_int,     # nEvents
    ctypes.c_void_p,  # dVtxOut
    ctypes.c_void_p,  # dMomOut
    ctypes.c_void_p,  # dChi2Out
]


def run_vertexing(tracks_glb, ntracks_glb, gplane_array):
    assert gplane_array.dtype == np.float32 and gplane_array.ndim == 1

    n_events = tracks_glb.shape[0]

    d_tracks = cp.asarray(tracks_glb, dtype=cp.float32)
    d_ntracks = cp.asarray(ntracks_glb, dtype=cp.uint32)
    d_plane = cp.asarray(gplane_array, dtype=cp.float32)

    d_vtx = cp.empty((n_events, 3), dtype=cp.float32)
    d_mom = cp.empty((n_events, 3), dtype=cp.float32)
    d_chi2 = cp.empty((n_events, 3), dtype=cp.float32)

    _lib_vtx.launch_gKernel_Vertexing(
        int(d_tracks.data.ptr),
        int(d_ntracks.data.ptr),
        int(d_plane.data.ptr),
        int(n_events),
        int(d_vtx.data.ptr),
        int(d_mom.data.ptr),
        int(d_chi2.data.ptr),
    ...
    return (cp.asnumpy(d_vtx), cp.asnumpy(d_mom), cp.asnumpy(d_chi2))


# ---------------- Dimuon ----------------
# Built from kernels/gkernel_dimuon_building.cu -> utils/libdimuon_building.so
_dimu_lib = None
_has_dimu = False
for _name in ("libdimuon_building.so", "libdimuon.so"):
    _path = os.path.join(os.path.dirname(__file__), _name)
    try:
        _dimu_lib = ctypes.CDLL(_path)
        _has_dimu = True
        break
    except OSError:
        pass
...
if _has_dimu:
    _dimu_lib.launch_gKernel_Dimuon_Building.restype = None
    _dimu_lib.launch_gKernel_Dimuon_Building.argtypes = [
        ctypes.c_void_p,  # dXZ
        ctypes.c_void_p,  # dNXZ (uint32*)
        ctypes.c_void_p,  # dYZ
        ctypes.c_void_p,  # dNYZ (uint32*)
        ctypes.c_void_p,  # dHits (SoA float*)
        ctypes.c_void_p,  # dNHits (int32*)
        ctypes.c_void_p,  # dPlane (float*)
        ctypes.c_void_p,  # dNDimu (int32*)
        ctypes.c_void_p,  # dDimVtx (float*)
        ctypes.c_void_p,  # dDimP   (float*)
        ctypes.c_void_p,  # dDimMass (float*)
        ctypes.c_void_p,  # dDimChi2 (float*)
        ctypes.c_void_p,  # dMu1PQ (float*)
        ctypes.c_void_p,  # dMu2PQ (float*)
        ctypes.c_void_p,  # dXF (float*)
        ctypes.c_void_p,  # dX1 (float*)
        ctypes.c_void_p,  # dX2 (float*)
        ctypes.c_void_p,  # dPT (float*)
        ctypes.c_void_p,  # dPhi (float*)
        ctypes.c_void_p,  # dCosTheta (float*)
        ctypes.c_int,     # nEvents
        ctypes.c_float,   # z_kmag_bend_cm
        ctypes.c_float,   # pt_kick_kmag
        ctypes.c_float,   # d0_win_cm
        ctypes.c_float,   # mass_min
        ctypes.c_float,   # mass_max
        ctypes.c_int,     # require_opposite_charge
    ]


def run_dimuon_building(
    tracks_xz,
    ntracks_xz,
    tracks_yz,
    ntracks_yz,
    ...
    if not _has_dimu:
        raise RuntimeError(
            "Dimuon library not found. Build utils/libdimuon_building.so with `make`."
        )

    z_kmag_bend_cm = float(z_kmag_bend_cm)
    pt_kick_kmag = float(pt_kick_kmag)
    d0_win_cm = float(d0_win_cm)
    mass_min = float(mass_min)
    mass_max = float(mass_max)
    req_opp = 1 if require_opposite_charge else 0

    n_events = int(tracks_xz.shape[0])

    d_xz = cp.asarray(tracks_xz, dtype=cp.float32)
    d_nxz = cp.asarray(ntracks_xz, dtype=cp.uint32)

    d_yz = cp.asarray(tracks_yz, dtype=cp.float32)
    d_nyz = cp.asarray(ntracks_yz, dtype=cp.uint32)
    ...
    # outputs
    d_ndimu = cp.zeros((n_events,), dtype=cp.int32)
    d_vtx = cp.empty((n_events, 3), dtype=cp.float32)
    d_p = cp.empty((n_events, 3), dtype=cp.float32)
    d_mass = cp.empty((n_events,), dtype=cp.float32)
    d_chi2 = cp.empty((n_events,), dtype=cp.float32)
    d_mu1 = cp.empty((n_events, 4), dtype=cp.float32)
    d_mu2 = cp.empty((n_events, 4), dtype=cp.float32)

    d_xf = cp.empty((n_events,), dtype=cp.float32)
    d_x1 = cp.empty((n_events,), dtype=cp.float32)
    d_x2 = cp.empty((n_events,), dtype=cp.float32)
    d_pt = cp.empty((n_events,), dtype=cp.float32)
    d_phi = cp.empty((n_events,), dtype=cp.float32)
    d_costh = cp.empty((n_events,), dtype=cp.float32)

    _dimu_lib.launch_gKernel_Dimuon_Building(
        int(d_xz.data.ptr),
        int(d_nxz.data.ptr),
        int(d_yz.data.ptr),
        int(d_nyz.data.ptr),
